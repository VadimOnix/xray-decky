...
1. **Backend: Import HTTPS server**
   ...
   - **TLS certificate**: On first run or at startup, if cert/key missing under `DECKY_PLUGIN_RUNTIME_DIR`, generate self-signed cert (e.g. via `cryptography` or `openssl` subprocess); save `cert.pem`, `key.pem`. Create runtime dir if missing. On generation/load failure: log error, do not start import server.
   ...
   - Add `cryptography` (or document openssl-only) to `backend/requirements.txt`.
...
5. **Dependencies**
   - Backend: add `aiohttp` to Python dependencies (e.g. in plugin runtime or requirements).
