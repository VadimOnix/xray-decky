...
- **Page and assets**: `docs/plugin-page/`
...
1. Open `docs/plugin-page/index.html` for content (sections, text, image references).
2. Open `docs/plugin-page/styles/main.css` for layout and appearance (breakpoints, colors, typography).
3. Add or replace images in `docs/plugin-page/assets/` (hero, features). Keep `alt` text updated in HTML.
4. Keep content in sync with `plugin.json` (name, description) and README (features, install) when you change them.

## How to preview locally

1. Open `docs/plugin-page/index.html` in a browser (file:// or via a local static server).
...
- **GitHub Pages** (recommended): Enable Pages for the repo, source = branch (e.g. `main`) and folder = `/docs`. The page will be at `https://<owner>.github.io/<repo>/plugin-page/`. The URL is stable (path does not change; no session params).
- **Alternative**: Copy `docs/plugin-page/` to any static host; ensure `index.html` is served at the chosen path and assets use relative paths. Use a fixed path (e.g. `/plugin-page/`) for shareability.
