...
- **The surfaces**: GitHub repo description + topics (live, via `gh api` — maintainer sign-off required), `plugin.json` `publish.description` (plugin metadata), `package.json` description/keywords, the five READMEs, and the site dictionaries (`site/src/i18n/*.ts`). A wording change lands on all relevant surfaces in the same change, not "later".
- **Distribution**: the plugin is **not** in the Decky Plugin Store (the Decky maintainers declined it over GPL licensing concerns about the proxy cores). Install instructions everywhere list exactly two channels: the installer script (`scripts/install-xray-decky.sh` / the `.desktop` one-click wrapper) and the release zip via Decky → Settings → Developer → Install Plugin from URL.
